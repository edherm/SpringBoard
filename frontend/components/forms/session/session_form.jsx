...
class SessionForm extends React.Component {
  ...
  fillForm(e) {
    e.preventDefault();
    this.props
      .action({ email: 'demo@email.com', password: 'password' })
      .then((user) => {
        this.props.history.push(`/${user.id}/projects`);
      });
  }
      ...
      if (formType === 'Log In') {
        return (
          <div className="login-header-container">
            <h2 className={`login-header`}>{formType} to SpringBoard</h2>
          </div>
        );
      } else {
        return (
    ...
    const emailInput = () => (
      <>
        <label
          htmlFor="input-form"
          className={
            this.state.selected.email ? 'filled session-label' : 'session-label'
          }
        >
        ...
        <div className={formTypeKlass}>
          ...
          <div className="session-form-container">
            ...
            <form className="session-form" onSubmit={this.handleSubmit}>
              {formOption()}
              <div className="session-input-container">{emailInput()}</div>
              <div className="session-input-container">{passwordInput()}</div>
              <ul>{errorMessages()}</ul>
              ...
              <div className="switch-session-form-link-wrapper">
                <Link
                  className="switch-session-form-link"
