...
export const fetchMessages = projectId => dispatch => {
  return messageAPIUtil.fetchMessages(projectId)
    .then(messages => dispatch(receiveMessages(messages)))
}

export const fetchMessage = (projectId, messageId) => dispatch => {
  return messageAPIUtil.fetchMessage(projectId, messageId)
    .then(message => dispatch(receiveMessage(message)))
}

export const createMessage = (projectId, message) => dispatch => {
  return messageAPIUtil.createMessage(projectId, message)
    .then(message => {
...
export const updateMessage = (projectId, message) => dispatch => {
  return messageAPIUtil.updateMessage(projectId, message)
    .then(message => {
      return dispatch(receiveMessage(message));
    }, errors => {
      return dispatch(receiveMessageErrors(errors.responseJSON));
...
export const deleteMessage = (projectId, messageId) => dispatch => {
  return messageAPIUtil.deleteMessage(projectId, messageId)
